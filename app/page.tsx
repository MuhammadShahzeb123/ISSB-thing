      ...
      <div className="mx-auto max-w-6xl">
        <header className="max-w-4xl">
          ...
          <p className="mt-6 max-w-3xl text-lg font-semibold leading-8 text-slate-700">
            Official ISSB material describes Psychological, GTO, and Deputy President/interview as the
            core assessor dimensions. Physical and General Knowledge are included here as preparation
            areas, not additional official dimensions.
          </p>
        </header>
          ...
          {siteNavigation.map((area) => (
            <Link
              ...
              className={`border-2 border-slate-950 p-6 text-slate-950 shadow-[6px_6px_0_#171717] transition hover:-translate-y-1 focus-visible:-translate-y-1 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-blue-700 ${
                area.slug === 'general-knowledge' ? 'bg-amber-100 lg:col-start-3' : 'bg-white'
              }`}
            >
...
export default function HomePage({ searchParams }: HomePageProps) {
  const { practice } = use(searchParams);
  return practice === 'wat' ? <WATPractice /> : <HomeLanding />;
}
